...
type ExtendedPostApiType = PostApiType & { is_liked: boolean };

export default function PostCard({ post, onLike }: { post: ExtendedPostApiType; onLike: (postId: number) => void }) {
  const copyUrl = () => {
      ...
      <Card className="w-[330px] h-[450px] bg-muted">
        ...
        <CardFooter className="flex justify-between items-center">
          <ArrowBigUp
            size={25}
            onClick={handleLike}
            className={`cursor-pointer ${post.is_liked ? 'text-green-500' : ''}`}
          />
